  ...
  .detail
    .form 
      ...
      .item
        ...
        input(v-model="user.mobile",type="number",placeholder="*为领取话费红包的手机")
...
const info = reactive({ name: '', address: '' })

const submit = async () => {
  if (!info.name) return showToast('请填写姓名')
  if (!user.mobile) return showToast('请填写电话')
  ...
  try {
    ...
    const { data } = await axios(
      `https://marry.aienji01.cn/houtai_h1/account/act_submit?mobile=${user.mobile}&name=${info.name}&address=${info.address}`
    )
    showToast(data.msg)
    if (!data.code) {
      showToast(data.msg)
